...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center text-center sm:text-left gap-4 pb-5 border-b border-gray-100">
        <div>
            <h1 class="text-2xl md:text-3xl font-black text-gray-800 flex items-center justify-center sm:justify-start gap-2">
                <span class="text-green-600">👤</span> Editar Jugador
            </h1>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 max-w-2xl">
        <form method="POST" action="{{ route('admin.teams.players.update', [$team, $player]) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="number" name="dorsal" value="{{ old('dorsal', $player->dorsal) }}" min="1" max="99"
            ...
            <div>
                ...
                <select name="position"
                    ...
                    <option value="GK"  {{ old('position', $player->position) === 'GK'  ? 'selected' : '' }}>Portero (GK)</option>
                    <option value="DEF" {{ old('position', $player->position) === 'DEF' ? 'selected' : '' }}>Defensa (DEF)</option>
                    <option value="MID" {{ old('position', $player->position) === 'MID' ? 'selected' : '' }}>Mediocampista (MID)</option>
                    <option value="FWD" {{ old('position', $player->position) === 'FWD' ? 'selected' : '' }}>Delantero (FWD)</option>
                </select>
                @error('position')
                    <p class="text-red-500 text-xs font-medium mt-1.5 flex items-center gap-1">❌ {{ $message }}</p>
                @enderror
