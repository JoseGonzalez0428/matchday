...
@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-green-800">👤 Editar Jugador</h1>
    <p class="text-gray-500 mt-1">{{ $team->name }}</p>
</div>

<div class="bg-white rounded-xl shadow p-6 max-w-2xl">
    <form method="POST" action="{{ route('admin.teams.players.update', [$team, $player]) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="number" name="dorsal" value="{{ old('dorsal', $player->dorsal) }}"
                   min="1" max="99"
                   class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-green-500
