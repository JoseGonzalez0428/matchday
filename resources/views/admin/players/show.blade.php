...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center text-center sm:text-left gap-4 pb-5 border-b border-gray-100">
        ...
        <div class="flex gap-2.5 w-full sm:w-auto">
            <a href="{{ route('admin.teams.players.edit', [$team, $player]) }}"
               class="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs px-4 py-2.5 rounded-xl shadow-sm transition-all">
                Editar
            </a>
            <a href="{{ route('admin.teams.players.index', $team) }}"
               class="flex-1 sm:flex-none inline-flex items-center justify-center text-xs font-bold text-gray-600 hover:text-gray-800 bg-white border border-gray-200 px-4 py-2.5 rounded-xl shadow-sm transition-all whitespace-nowrap">
                ← Volver
            </a>
        </div>
    </div>

    {{-- Stats Generales (Estructura de 5 columnas del archivo original) --}}
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div class="bg-rose-50 border border-rose-100 rounded-2xl p-4 text-center shadow-xs">
            <p class="text-[10px] font-bold text-rose-600 uppercase tracking-wider">Autogoles</p>
            <p class="text-3xl font-black text-rose-800 mt-1">{{ $ownGoals }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-gray-100 p-4 text-center shadow-sm col-span-2 md:col-span-1">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-50 bg-slate-50/10">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors font-medium text-gray-600">
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                <span class="inline-flex px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border
                                    {{ $goal->type === 'regular' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : '' }}
                                    {{ $goal->type === 'penalty' ? 'bg-blue-50 text-blue-700 border-blue-100' : '' }}
                                    {{ $goal->type === 'own_goal' ? 'bg-rose-50 text-rose-700 border-rose-100' : '' }}">
                                    {{ \App\Helpers\StatusHelper::goalType($goal->type) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold font-mono tracking-tight shadow-xs {{ $resultColor }}">
