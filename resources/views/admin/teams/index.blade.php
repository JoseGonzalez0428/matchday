    ...
    <div class="flex flex-wrap gap-2">
        <form method="GET" action="{{ route('admin.teams.index') }}" class="flex gap-2">
            <input type="text" name="search" value="{{ $search ?? '' }}"
                   ...
                   class="border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 w-40 md:w-48">
            <button type="submit"
                    class="bg-green-100 text-green-700 px-3 py-2 rounded-lg text-sm hover:bg-green-200">
                🔍
            </button>
            @if($search)
            <a href="{{ route('admin.teams.index') }}"
               class="px-3 py-2 rounded-lg border text-sm hover:bg-gray-50 text-gray-600">
                ✕
            </a>
            @endif
        </form>
        <a href="{{ route('admin.teams.create') }}"
           class="bg-green-700 text-white px-3 py-2 rounded-lg text-sm hover:bg-green-800">
            + Nuevo
        </a>
    </div>
</div>
