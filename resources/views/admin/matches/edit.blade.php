...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-5 border-b border-gray-100">
    <div>
    ...
    <button type="button" id="predict-btn" onclick="getPrediction()"
            class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-semibold text-sm px-4 py-2.5 rounded-xl shadow-sm transition-all transform active:scale-95">
...
{{-- Bloque de Formulario Principal --}}
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm max-w-3xl overflow-hidden">
    <form method="POST" action="{{ route('admin.matches.update', $match) }}?from={{ request('from') }}&id={{ request('id') }}" class="p-6">
        @csrf
        @method('PUT')

        {{-- Grid del Marcador Global --}}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            {{-- Local --}}
            <div class="bg-slate-50/60 border border-gray-100 p-4 rounded-xl">
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">
                    ⚽ Goles Local — <span class="text-gray-800 font-black">{{ $match->homeTeam->name }}</span>
                </label>
                <input type="number" name="home_score" min="0" max="99" placeholder="0"
                       value="{{ old('home_score', $match->home_score) }}"
                       class="w-full bg-white border border-gray-200 text-center font-black text-xl rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all shadow-sm @error('home_score') border-red-500 ring-2 ring-red-100 @enderror">
                @error('home_score')
            ...
            <div class="bg-slate-50/60 border border-gray-100 p-4 rounded-xl">
                <label class="block text-xs font-bold uppercase tracking-wider text-gray-500 mb-2">
                    ⚽ Goles Visitante — <span class="text-gray-800 font-black">{{ $match->awayTeam->name }}</span>
                </label>
                <input type="number" name="away_score" min="0" max="99" placeholder="0"
                       value="{{ old('away_score', $match->away_score) }}"
                       class="w-full bg-white border border-gray-200 text-center font-black text-xl rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all shadow-sm @error('away_score') border-red-500 ring-2 ring-red-100 @enderror">
                @error('away_score')
                    <p class="text-red-500 text-xs font-medium mt-1.5 flex items-center gap-1">❌ {{ $message }}</p>
                @enderror
            </div>
        ...
        <div id="penalties-section" class="{{ ($match->home_score === $match->away_score && $match->status === 'finished') ? '' : 'hidden' }} bg-blue-50/60 border border-blue-100 rounded-2xl p-5 mb-6 shadow-inner">
            <div class="flex items-center gap-2 mb-2">
                <span class="text-lg">🎯</span>
                <h3 class="font-bold text-blue-800 text-sm tracking-tight">Definición por Penales Obligatoria</h3>
            </div>
            <p class="text-blue-600 text-xs mb-4">El partido eliminatorio se encuentra empatado. Registra la tanda reglamentaria.</p>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Penales {{ $match->homeTeam->name }}</label>
                    <input type="number" name="home_penalties" min="0" max="99" placeholder="0"
                           value="{{ old('home_penalties', $match->home_penalties) }}"
                           class="w-full bg-white border border-gray-200 font-bold rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1.5">Penales {{ $match->awayTeam->name }}</label>
        ...
        <div class="mb-6 border-t border-gray-100 pt-5">
            <div class="flex justify-between items-center mb-4">
                ...
                <button type="button" onclick="addGoal()" id="add-goal-btn"
                        class="inline-flex items-center gap-1 bg-green-50 hover:bg-green-100 text-green-700 font-semibold border border-green-200 px-3 py-1.5 rounded-lg text-xs shadow-sm transition-all">
                    ➕ Agregar gol
                </button>
            </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 border-t border-gray-100 pt-5">
            ...
            <a href="{{ route('admin.matches.show', $match) }}" class="w-full sm:w-auto px-6 py-2.5 rounded-xl border border-gray-200 hover:bg-gray-50 text-gray-600 font-semibold text-sm text-center transition-all">
                Cancelar
            </a>
        </div>
    </form>
